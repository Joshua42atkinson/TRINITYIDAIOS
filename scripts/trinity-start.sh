...
# SEQUENTIAL LOADING: Ensures llama-server is HEALTHY
...
MAX_WAIT=120  # seconds to wait for llama-server
...
if [ "$STARTED" = false ]; then
    echo "[Trinity Startup] ✗ ERROR: llama-server not healthy after ${MAX_WAIT}s"
