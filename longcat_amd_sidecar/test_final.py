...
while time.time() - start < 300:
    try:
        ...
        if res.json().get('loaded'):
            ...
            x = requests.post('http://127.0.0.1:8010/v1/chat/completions', json={'messages': [{'role':'user', 'content':'hello'}], 'max_tokens':20})
            print(json.dumps(x.json(), indent=2))
            
            print("\nTesting Image Gen MOCK (just checking no 500 error)...")
            y = requests.post('http://127.0.0.1:8010/v1/images/generations', json={'prompt':'a cat'})
            print("Status:", y.status_code)
