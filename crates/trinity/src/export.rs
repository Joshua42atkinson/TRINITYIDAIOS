...
/// Export a professional DOCX portfolio document
///
/// Students download this as their deliverable — works in Word, Google Docs, LibreOffice.
/// Contains: title, metadata, PEARL summary, quest progress, objectives, vocabulary.
fn export_docx_portfolio(container: &EyeContainer) -> (String, Vec<u8>, &'static str) {
    let meta = &container.metadata;
    let summary = &container.quest_summary;
    ...
    docx = docx.add_paragraph(
        Paragraph::new()
            .add_run(
                Run::new()
                    .add_text(format!("Subject: {} — Generated by Trinity ID AI OS", meta.subject))
                    .size(24) // 12pt
                    .color("666666"),
            ),
    );
    ...
    let meta_lines = [
        ...
        ("Grade Level", if meta.grade_level.is_empty() { "Not set" } else { &meta.grade_level }),
        ("PEARL Phase", &meta.pearl_phase),
        ("Alignment Grade", &meta.alignment_grade),
    ...
    docx = docx.add_paragraph(
        Paragraph::new()
            .add_run(Run::new().add_text(&container.pearl_summary).size(22)),
    );
    ...
    let progress_lines = [
        format!("Phases Completed: {} / {}", summary.phases_completed, summary.total_phases),
        format!("Total XP: {}", summary.total_xp),
        format!("Coal Used: {:.1}", summary.coal_used),
        format!("Steam Generated: {:.1}", summary.steam_generated),
    ];
    for line in &progress_lines {
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text(format!("• {}", line)).size(22)),
        );
    }

    // === Objectives ===
    if !container.objectives.is_empty() {
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("").size(20)),
        );
        ...
        for obj in &container.objectives {
            let check = if obj.completed { "☑" } else { "☐" };
            docx = docx.add_paragraph(
                Paragraph::new()
                    .add_run(
                        Run::new()
                            .add_text(format!("{} [{}] ", check, obj.phase))
                            .bold()
                            .size(22)
                            .color(if obj.completed { "2E7D32" } else { "888888" }),
                    )
                    .add_run(Run::new().add_text(&obj.description).size(22)),
            );
        }
    }

    // === Vocabulary ===
    if !container.vocabulary.is_empty() {
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("").size(20)),
        );
        ...
        for vocab in &container.vocabulary {
            docx = docx.add_paragraph(
                Paragraph::new()
                    .add_run(Run::new().add_text(format!("{}  — ", vocab.word)).bold().size(22))
                    .add_run(Run::new().add_text(&vocab.definition).size(22)),
            );
        }
    }
    ...
    docx = docx.add_paragraph(
        Paragraph::new()
            .add_run(
                Run::new()
                    .add_text("Built with Trinity ID AI OS — The Iron Road")
                    .size(18)
                    .italic()
                    .color("999999"),
            ),
    ...
    // Build the DOCX binary
    let mut buf = std::io::Cursor::new(Vec::new());
    let _ = docx.build().pack(&mut buf);
    let bytes = buf.into_inner();

    let filename = format!("{}_portfolio.docx", slug(&meta.title));
    (
        filename,
        bytes,
