...
export default function InferenceManager() {
  ...
  // Fetch focus mode status
  const fetchFocusMode = useCallback(async () => {
    try {
      ...
      // Focus API may not be available on older builds
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const focusInterval = setInterval(fetchFocusMode, 15000); // Poll focus every 15s
    return () => { clearInterval(interval); clearInterval(focusInterval); };
  }, [fetchResources, fetchFocusMode]);
  ...
  const handleFocusSwitch = useCallback(async (mode) => {
    setFocusBusy(true);
    try {
      const res = await fetch(`/api/focus/${mode}`, { method: 'POST' });
      const data = await res.json();
      console.log(`Focus mode: ${mode}`, data);
      setFocusMode(mode);
      window.dispatchEvent(new CustomEvent('trinity-toast', {
        detail: { message: data.message || `Switched to ${mode} focus`, type: 'success' }
      }));
      // Refresh resources after mode switch
      setTimeout(() => { fetchResources(); fetchFocusMode(); }, 3000);
    } catch (err) {
      console.error(`Focus switch failed:`, err);
      window.dispatchEvent(new CustomEvent('trinity-toast', {
        detail: { message: `Focus switch failed: ${err.message}`, type: 'error' }
      }));
    } finally {
      setFocusBusy(false);
    }
  ...
  return (
    <div className="im-container">
      ...
      {/* Focus Mode Controls */}
      <div className="im-focus-bar">
        <span className="im-focus-label">FOCUS MODE</span>
        <button
          className={`im-focus-btn ${focusMode === 'creative' ? 'im-focus-btn--active' : ''}`}
          onClick={() => handleFocusSwitch('creative')}
          disabled={focusBusy}
          title="Kill IDEs, keep Trinity P+A running for creative work"
        >
          🎨 Creative
        </button>
        <button
          className={`im-focus-btn ${focusMode === 'code' ? 'im-focus-btn--active' : ''}`}
          onClick={() => handleFocusSwitch('code')}
          disabled={focusBusy}
          title="Kill Trinity models, free VRAM for IDE agents"
        >
          💻 Code
        </button>
        <button
          className={`im-focus-btn ${focusMode === 'night' ? 'im-focus-btn--active' : ''}`}
          onClick={() => handleFocusSwitch('night')}
          disabled={focusBusy}
          title="Kill all heavy processes"
        >
          🌙 Night
        </button>
        {focusBusy && <span className="im-focus-spinner" />}
      </div>
