...
use trinity_protocol::pearl::{PearlMedium, PearlPhase};
...
// ═══════════════════════════════════════════════════════════════════════════════
// COMMAND QUEUE — Bevy Resource for receiving commands from Pete
// ═══════════════════════════════════════════════════════════════════════════════

#[cfg(feature = "desktop")]
use bevy::prelude::*;

/// Bevy resource: queue of pending DAYDREAM commands (blueprints or individual).
/// Pete's agent tools push here; Bevy systems consume each frame.
#[cfg(feature = "desktop")]
#[derive(Resource, Default)]
pub struct DaydreamCommandQueue {
    pub blueprints: Vec<DaydreamBlueprint>,
    pub commands: Vec<DaydreamCommand>,
}

#[cfg(feature = "desktop")]
impl DaydreamCommandQueue {
    /// Push a full blueprint (batch of commands from a phase transition).
    pub fn push_blueprint(&mut self, blueprint: DaydreamBlueprint) {
    ...
    /// Drain all pending blueprints.
    pub fn drain_blueprints(&mut self) -> Vec<DaydreamBlueprint> {
        std::mem::take(&mut self.blueprints)
    }

    /// Drain all pending individual commands.
    pub fn drain_commands(&mut self) -> Vec<DaydreamCommand> {
        std::mem::take(&mut self.commands)
    }

    /// Parse a JSON string into a command and queue it.
    pub fn push_json(&mut self, json: &str) -> Result<(), serde_json::Error> {
        let cmd: DaydreamCommand = serde_json::from_str(json)?;
        self.commands.push(cmd);
        Ok(())
    }

    /// Parse a JSON string into a blueprint and queue it.
    pub fn push_blueprint_json(&mut self, json: &str) -> Result<(), serde_json::Error> {
        let bp: DaydreamBlueprint = serde_json::from_str(json)?;
...
// ═══════════════════════════════════════════════════════════════════════════════
// ECS MARKERS — Components for DAYDREAM entities
// ═══════════════════════════════════════════════════════════════════════════════

/// Marker component for entities managed by DAYDREAM.
#[cfg(feature = "desktop")]
#[derive(Component)]
pub struct DaydreamEntity {
    pub id: DaydreamEntityId,
    /// Which ADDIECRAPEYE station this entity belongs to
    pub station: Option<u8>,
}

/// Marker for the Yardmaster's avatar.
#[cfg(feature = "desktop")]
#[derive(Component)]
...
/// Marker for subject-specific NPCs.
#[cfg(feature = "desktop")]
#[derive(Component)]
...
/// Marker for quest waypoints in the world.
#[cfg(feature = "desktop")]
#[derive(Component)]
...
/// Component for entities smoothly moving toward a target.
#[cfg(feature = "desktop")]
#[derive(Component)]
pub struct MoveTo {
